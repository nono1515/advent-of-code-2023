...
fn part2(input: &str) -> u64 {
    let corners = parse_input(input);
    let x_mapping = shrink(&corners, 0);
    let y_mapping = shrink(&corners, 1);
    let corners_mapped = corners
        .iter()
        .map(|c| [x_mapping[&c[0]], y_mapping[&c[1]]])
        .collect::<Vec<_>>();

    let xsize = corners_mapped.iter().map(|c| c[0]).max().unwrap() as usize;
    let ysize = corners_mapped.iter().map(|c| c[1]).max().unwrap() as usize;
    let mut borders = vec![vec![Wall::Empty; xsize + 1]; ysize + 1];
    for ((c1, c2), c3) in corners_mapped
        .iter()
        .zip(corners_mapped.iter().cycle().skip(1))
        .zip(corners_mapped.iter().cycle().skip(2))
    {
...
fn shrink(corners: &[Tile], index: usize) -> HashMap<u64, u32> {
    let mut axis = corners.iter().map(|c| c[index]).collect::<Vec<_>>();
    axis.push(u64::MIN);
    axis.push(u64::MAX);
    axis.sort_unstable();
    axis.dedup();
