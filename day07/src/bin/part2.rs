...
fn get_type(cards: &[Card; 5]) -> Type {
    ...
    // println!("{:?}", card_counts);
    if let Some(joker_count) = card_counts.remove(&Card::Jocker) {
        let max_val_key = card_counts
            .iter()
            .max_by(|a, b| a.1.cmp(b.1))
            .map(|(k, _)| *k)
            .unwrap_or(&Card::Ace); // In case all values are jokers
                                    // println!("{:?} {:?}", max_val_key, card_counts);
        card_counts.entry(max_val_key).or_insert(0);
        *card_counts.get_mut(max_val_key).unwrap() += joker_count;
    }
    // println!("{:?}", card_counts);
